...
class EnrolmentTypes
{
    ...
    const TYPE_AWARD             = 'award'; // Group of courses with a rule so that learner can achieve it, data in award_enrolment table.
    const TYPE_VIRTUAL_ENROLMENT = 'virtual-enrolment'; // Not started enrolment created for a learner in a learning object, there are no data in database.

    const ALL = [self::TYPE_ENROLMENT, self::TYPE_MANUAL_RECORD, self::TYPE_PLAN_ASSIGNED, self::TYPE_AWARD, self::TYPE_VIRTUAL_ENROLMENT];
}
